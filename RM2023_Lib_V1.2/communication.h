#ifndef COMMUNICATION_H
#define COMMUNICATION_H

#ifdef __cplusplus
 extern "C" {
#endif
	 
#include "main.h"
#include "string.h"
typedef unsigned char uc;

#define MINI_PC_BUF_SIZE 		  128		// 数组大小，可修改

#define Communication_Mode  2
#define Communication_huart 1
#define Communication_USB_VCP 2
#define Communication_USB_HID 3	 
	 
	 
#if Communication_Mode == Communication_huart
	 
#include "usart.h"
	 
#define MINI_PC_USART_HANDLE 		huart3// 串口号，可修改
extern uc Mini_PC_info_ubuf[MINI_PC_BUF_SIZE];
#define Mini_PC_INFO(...) HAL_UART_Transmit (&MINI_PC_USART_HANDLE,\
													(uint8_t *)Mini_PC_info_ubuf,\
													sprintf((char *)Mini_PC_info_ubuf,__VA_ARGS__),\
													0xffff)
													
#elif Communication_Mode == Communication_USB_VCP 

//#include "usbd_cdc.h"
//#include "usbd_cdc_if.h"

#elif Communication_Mode == Communication_USB_HID

#include "usbd_customhid.h"
#include "usbd_custom_hid_if.h"
extern USBD_HandleTypeDef hUsbDeviceFS;

#endif

extern uc Mini_PC_tx_buf[128];
extern uc Mini_PC_rx_buf[128];
													
typedef union
{
    float f;
    uc c[4];
} _angle;

typedef union
{
    float f;
    uc c[4];
} _data;

typedef struct
{
    uc mine;                            //己方颜色: 红/蓝  1/0
		uc buff_status;								      //符/普通      1    0 
	  uc zimiao_status;                   //自瞄      1  0
		uc close_PC_status;                 //关小电脑  1 0
		uc adjust_camera;										//调整相机    1 0
	  uc shooter_speed_limit;	            //上限射速
	  _data Yaw_sensor_data_mang;         //Y轴角度
	  _data pitch_mang;
}_request;

typedef struct
{
    _angle pitch;
    _angle yaw;
		uc identify_target;		//识别到装甲板	
		uc identify_buff;			//识别打打符
    float distance;    //射击模式  ：自瞄/能量机关
}_response;

static const unsigned char crc_table[] =
{
    
};

//数据收发单体
extern volatile _request request;
extern volatile _response  response;

extern uc cal_crc_table(uc *ptr, uc len);
extern void Mini_PC_Init(void);
extern void Mini_PC_SendData();
extern void getReceiveData(uc (*buf));
extern uc cal_crc_table(uc *ptr, uc len);

#ifdef __cplusplus
}
#endif

#endif
