...
#define INFO(...)                                                              \
  HAL_UART_Transmit(&PRINTF_USART_HANDLE, (uint8_t *)info_ubuf,                \
                    sprintf((char *)info_ubuf, __VA_ARGS__), 0xffff)
...
public:
	typedef enum{
		USER_CAN_RUNING_START = 0,
		USER_CAN_FIFO_ERROR,
		USER_CAN_VOLTAGE_ERROR,
		USER_CAN_FREENUM_ERROR,
		}user_can_error_status_t;
  ...
  HAL_StatusTypeDef Send_RM(uint16_t Id, int16_t M_201, int16_t M_202,
                            int16_t M_203, int16_t M_204);
  ...
  HAL_StatusTypeDef RMD_Write_PID(uint16_t Id, uint16_t Order_Id,
                                  uint16_t anglePidKp, uint16_t anglePidKi,
                                  uint16_t speedPidKp, uint16_t speedPidKi,
                                  uint16_t iqPidKp, uint16_t iqPidKi);
  HAL_StatusTypeDef RMD_Write_ACCLE_to_RAM(uint16_t Id, int32_t Accel);
  HAL_StatusTypeDef
  RMD_Write_EncoderOffset_to_ROM(uint16_t Id,
                                 uint16_t EncoderOffset); //写入编码器值
  ...
  HAL_StatusTypeDef RMD_Speedcontrol_Motor(uint16_t Id, uint16_t Order_Id,
                                           uint16_t maxSpeed,
                                           int32_t angleControl);
  HAL_StatusTypeDef RMD_Anglecontrol_Motor(uint16_t Id, uint16_t Order_Id,
...
public:
  ...
  uint16_t ZERO_0[4] = {
      4758, 2060, 3415,
      7417}; // qz,hz,hy,qy  每个舵轮都不一样，自己修改 6100,6200,2020,6170 正常
             // 6156,6101,2093,6087 好一点//6141,6157,2020,6147
             // 很好但是不正常的1991
  uint16_t ZERO_45[4] = {5782, 3087, 4400,
                         249}; // qz,hz,hy,qy  每个舵轮都不一样，自己修改 //7150
...
#define Yaw_mid_of_45 3076
  ...
  void Not_Xiaotuoluo_Jie_Suan(float CH0, float CH1, int16_t CH2,
                               int16_t Angle_L, int16_t SPEED_MAX);
  void Xiaotuoluo_jie_Suan(uint16_t Mang_yaw, int16_t CH0, int16_t CH1,
                           int16_t CH2, int16_t Angle, int16_t SPEED_MAX);
...
public:
  ...
  void adxrs290_readBytes(uint8_t subAddress, uint8_t count,
                          uint8_t *spi_rev_buf);

  ADXRS290(SPI_HandleTypeDef *q, GPIO_TypeDef *w, uint16_t e, uint16_t t,
           float y, const char *u)
      : hspi(q), GPIOx(w), GPIO_Pin(e), SELF_TEST_NUM_290(t), DEAD_ZONE_290(y),
        string_check_290(u) {}
...
public:
  ...
  ADXRS453(SPI_HandleTypeDef *q, GPIO_TypeDef *w, uint16_t e,
           TIM_HandleTypeDef *r, uint16_t t, float y, const char *u)
      : hspi(q), GPIOx(w), GPIO_Pin(e), htim(r), SELF_TEST_NUM(t), DEAD_ZONE(y),
        string_check(u) {}
...
public:
  ...
  BMI088(SPI_HandleTypeDef *q, TIM_HandleTypeDef *t, GPIO_TypeDef *w1,
         uint16_t p1, GPIO_TypeDef *w2, uint16_t p2, uint16_t num, float dz,
         BMI088_GyroRangeTypeDef gyrorange, BMI088_AccRangeTypeDef accrange)
      : hspi(q), htim(t), CSB1_GPIOx(w1), CSB1_GPIO_Pin(p1), CSB2_GPIOx(w2),
        CSB2_GPIO_Pin(p2), SELF_TEST_NUM(num), dead_zoom(dz),
        GyroRange(gyrorange), AccRange(accrange) {}
...
private:
  ...
  float last_gyro_x, last_gyro_y, last_gyro_z, last_temperature, filter_count_x,
      filter_count_y, filter_count_z, filter_count_temperature;
  ...
  void BMI088_writeByte(GPIO_TypeDef *GPIOx, uint16_t GPIO_Pin,
                        uint8_t subAddress, uint8_t data);
  void BMI088_readBytes(GPIO_TypeDef *GPIOx, uint16_t GPIO_Pin,
                        uint8_t subAddress, uint8_t len, uint8_t *spi_rev_buf);
...
public:
  ...
  MCL_snail(TIM_HandleTypeDef *htim, TIM_HandleTypeDef *htim_z,
            uint32_t Channel_z, TIM_HandleTypeDef *htim_y, uint32_t Channel_y,
            uint8_t grade_1, uint8_t grade_2, uint8_t grade_3,
            uint8_t grade_1_error, uint8_t grade_2_error, uint8_t grade_3_error,
            const char *string_check)
      : htim(htim), htim_z(htim_z), Channel_z(Channel_z), htim_y(htim_y),
        Channel_y(Channel_y), grade_1(grade_1), grade_2(grade_2),
        grade_3(grade_3), grade_1_error(grade_1_error),
        grade_2_error(grade_2_error), grade_3_error(grade_3_error),
...
#define NUM                                                                    \
  (24 * PIXEL_NUM +                                                            \
   300) // Reset 280us（复位时间） / 1.25us = 224 NUM的值为单个灯珠的位宽（24）*
        // 灯珠数量（PIXEL_NUM）+ 复位脉冲数    1/84M = 11.9ns
...
public:
  ...
  RGB_UI(TIM_HandleTypeDef *htim, uint32_t Channel,
