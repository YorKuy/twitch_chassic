 ...
 * This file declares HiPNUC decoder functions and data structures.
 * It provides interfaces for decoding HiPNUC IMU/INS messages.
 */
...
#define HIPNUC_MAX_RAW_SIZE (512)
    ...
    /**
     * Packet 0x91: IMU data (floating point), 数据包0x91：IMU数据（浮点）
     */
    typedef struct __attribute__((__packed__))
    {
        uint8_t tag;          /* Data packet tag; if tag = 0x00, this packet is null（空包） */
     ...
     * 数据包0x81：INS数据，包含 lat、lon、eul、quat 和原始IMU数据
     */
    typedef struct __attribute__((__packed__))
    {
